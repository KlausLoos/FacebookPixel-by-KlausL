...
{
  "displayName": "Facebook Pixel v2",
  "description": "This is an unofficial Google Tag Manager template for the Facebook Pixel. Originally developed by Simo Ahava, forked on the JabJab version.\n\nSupport for GA4, custom-made for Luxury for Men.",
  "securityGroups": [],
  "categories": [
    "ADVERTISING",
    "ANALYTICS"
...
const parseEecObj = prod => {
  return {
    ...
    item_name: prod.item_name || '',  // Updated
    item_brand: prod.item_brand || '',  // Updated
    item_category: prod.item_category || '',  // Updated
    item_list_name: prod.item_list_name || '',  // Updated
    item_list_id: prod.item_list_id || '',  // Updated
    item_stock_status: prod.item_stock_status || '',  // Updated
    item_sale_product: prod.item_sale_product || '',  // Updated
    item_reviews_count: prod.item_reviews_count || 0,  // Updated
    item_reviews_score: prod.item_reviews_score || 0  // Updated
  };
};
...
if (data.enhancedEcommerce) {
  ...
  if ("view_item" === lastEventName) { eventName = 'ViewContent'; }
    ...
    return fail('Facebook Pixel: Most recently pushed ecommerce event name unsupported: ' + lastEventName);
  }
}


// Check for valid items array and purchase object
let itemsArray, isPurchaseEvent;

if (ecommerce && ecommerce.items && getType(ecommerce.items) === 'array') {
  itemsArray = ecommerce.items;
} else if (ecommerce && ecommerce.action && ecommerce.action.items && getType(ecommerce.action.items) === 'array') {
  itemsArray = ecommerce.action.items;
} else if (ecommerce && ecommerce.purchase && ecommerce.purchase.items && getType(ecommerce.purchase.items) === 'array') {
  itemsArray = ecommerce.purchase.items;
  isPurchaseEvent = true;
} else {
  return fail('Facebook pixel: No valid "items" array found in "ecommerce", "ecommerce.action", or "ecommerce.purchase" objects.');
}

if (itemsArray.length === 0) return fail('Facebook pixel: No product data found in the "items" array.');


eecObjectProps = {
  content_type: 'product',
  contents: itemsArray.map(parseEecObj),
  value: isPurchaseEvent ? makeNumber(ecommerce.purchase.value) : itemsArray.reduce((acc, cur) => {
...
if (['InitiateCheckout', 'Purchase'].indexOf(eventName) > -1) {
  ...
  if (eventName === 'InitiateCheckout') {
    itemsToReduce = ecommerce.items;
  } else if (eventName === 'Purchase') {
    itemsToReduce = ecommerce.purchase.items;
  }

  eecObjectProps.num_items = itemsToReduce.reduce((acc, cur) => {
